...
export function DimensionBars({ dimensionScores, showFixItLinks, onFixItClick, previousScores }: DimensionBarProps) {
  ...
  return (
    <div className="space-y-3">
      {sorted.map((dim, i) => (
        ...
        >
          {(() => {
            const prev = previousScores?.find((p) => p.dimensionId === dim.dimensionId)
            const delta = prev != null ? Math.round(dim.rawScore) - Math.round(prev.rawScore) : null
            return (
